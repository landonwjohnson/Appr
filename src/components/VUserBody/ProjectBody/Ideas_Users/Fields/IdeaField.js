...
class IdeaField extends Component {
    ...
    render() {
        const { index, ideaid, ideaData, handleChangeField, submitChangeField, handleDeleteField, handleSaveChange } = this.props;
        const field = 'ideas';
        ...
        return (
            <div className="ideas-item">
                <section>
                    <label>{(index + 1) + '.'}</label>
                    <input id={ideaid} name="idea_data" value={ideaData} onChange={e => handleChangeField(e, field, index)} onBlur={e => handleSaveChange(e, field, index)}/>
                </section>
